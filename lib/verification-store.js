...
function normalizeState(value) {
    ...
    return {
        ...
        shortLinks: (value.shortLinks && typeof value.shortLinks === 'object')
            ? value.shortLinks
            : base.shortLinks
    };
}
...
async function loadVerificationState({ requireRemote = false } = {}) {
    try {
    ...
    if (requireRemote) throw new Error('Shared verification state is unavailable');
...
function sanitizeVerificationState(state, silverCandidates = []) {
    const normalized = normalizeState(state);
    ...
    return {
        ...
        dobVerifications: require('./public-dob').publicDobVerifications(normalized.dobVerifications, silverCandidates)
    };
}
