...
function buildSilverMessageBody({ candidate, links }) {
    const sections = [
        ...
        `Please confirm your date of birth here: ${links.verify} Enter your date of birth and email address, then open the confirmation link emailed to you. Your date of birth is needed to determine your age when you achieved Silver C.`
    ];
    if (candidate.bilingual) sections.push(
        ...
        `Veuillez confirmer votre date de naissance ici : ${links.verify} Saisissez votre date de naissance et votre adresse courriel, puis ouvrez le lien de confirmation recu par courriel. Votre date de naissance permet de determiner votre age lors de l'obtention de l'insigne d'argent.`
    );
