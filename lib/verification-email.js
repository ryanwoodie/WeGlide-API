...
function formatVerificationSummary(details) {
    if (details.type === 'silver-dismiss') return 'I received my Silver badge before this season. Remove me from the Silver C-Gull candidate list.';
    if (details.type === 'pic') {
...
function buildMessage(details) {
    ...
    const subject = details.type === 'silver-dismiss'
        ? `Confirm Silver C-Gull removal for ${details.pilotName}`
        : details.type === 'pic'
