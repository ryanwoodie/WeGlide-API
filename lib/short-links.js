...
function createShortLinksForTargets({
    ...
    Object.entries(targets).forEach(([purpose, targetUrl]) => {
        ...
        shortened[purpose] = `${baseUrl}/api/verify-pic-hours?c=${encodeURIComponent(code)}`;
...
function isAllowedShortLinkTarget(targetUrl, baseUrl = process.env.PUBLIC_BASE_URL || 'https://sac-leaderboard.vercel.app') {
    try {
        const parsed = new URL(targetUrl);
        const expected = new URL(String(baseUrl || '').replace(/\/+$/, ''));
