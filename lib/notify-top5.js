...
function buildMessageBody({ candidate, links, seasonStartLabel = getSeasonStartLabel() }) {
    ...
    const english = `Hi ${candidate.firstName} | You're currently in the top 5 of the ${boardsDescription} under-200hr standings. To keep the leaderboard accurate, please confirm your PIC hours as of ${seasonStartLabel}: 1. Under 200 hrs PIC - verify here: ${links.verifyLink} | 2. Over 200 hrs PIC - clicking this link will remove you from the under-200 list: ${links.dismissLink} | View your standing: ${links.leaderboardLink} | Ryan Wood, SAC Sporting Committee`;

    if (!candidate.bilingual) {
        return english;
    }

    return `${english} | FR: Bonjour ${candidate.firstName} | Vous êtes actuellement dans le top 5 du classement moins de 200 h (${boardsDescription}). Pour garder le classement exact, veuillez confirmer vos heures PIC au ${seasonStartLabel}: 1. Moins de 200 h PIC - confirmez ici: ${links.verifyLink} | 2. Plus de 200 h PIC - cliquer ce lien vous retirera de la liste moins de 200 h: ${links.dismissLink} | Consultez votre classement: ${links.leaderboardLink} | Ryan Wood, SAC Sporting Committee`;
}
