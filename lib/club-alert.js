...
function buildNoClubAlertMessage({ pilotName, bilingual = false } = {}) {
    ...
    const english = [
        `Hi ${firstName}, Thanks for uploading your flight to WeGlide.`,
        `Your WeGlide profile does not currently have a club assigned. To qualify for the SAC Decentralized Soaring Contest, please add your club here: ${PROFILE_SETTINGS_URL}`,
        'After your club is saved, future SAC leaderboard updates can include you in the club-based contest scoring.',
        'Ryan Wood, SAC Sporting Committee'
    ].join(MESSAGE_SECTION_GAP);
    ...
    const french = [
        `FR: Bonjour ${firstName}, Merci d'avoir televerse votre vol sur WeGlide.`,
        `Aucun club n'est actuellement associe a votre profil WeGlide. Pour etre admissible au concours decentralise de l'Association canadienne de vol a voile (SAC), veuillez ajouter votre club ici: ${PROFILE_SETTINGS_URL}`,
        'Une fois votre club enregistre, les prochaines mises a jour du classement SAC pourront vous inclure dans le classement par club.',
        'Ryan Wood, SAC Sporting Committee'
    ].join(MESSAGE_SECTION_GAP);

    return [english, french].join(MESSAGE_SECTION_GAP);
}
