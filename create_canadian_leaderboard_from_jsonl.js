...
async function processCanadianFlights() {
    ...
    try {
        ...
        try {
            const manualVerificationState = sanitizeVerificationState(await loadVerificationState(), silverCGullLeaderboard);
        ...
        pilotVerificationData = sanitizeVerificationState(pilotVerificationData, silverCGullLeaderboard);
        ...
        function isDobVerifiedEntry(entry) {
            return !!(entry && entry.verified && entry.eligible !== false);
        }
...
No maximum distance bonus\`,
        ...
        function showDetailModalBackdrop() {
            if (detailModalBackdrop) return detailModalBackdrop;
            ...
            detailModalBackdrop = backdrop;
            return backdrop;
        }

        function hideDetailModalBackdrop() {
        ...
        function applyParentViewportPositionTo(el, payload) {
            if (!el || !payload || !payload.iframeRect) return false;
            const iframeLeft = Number(payload.iframeRect.left);
            const iframeTop = Number(payload.iframeRect.top);
            const parentVW = Number(payload.parentViewportWidth);
            const parentVH = Number(payload.parentViewportHeight);
            if (!Number.isFinite(iframeLeft) || !Number.isFinite(iframeTop) ||
                !Number.isFinite(parentVW) || !Number.isFinite(parentVH)) {
                return false;
            }
            ...
            const centerXInChild = (parentVW / 2) - iframeLeft;
            const centerYInChild = (parentVH / 2) - iframeTop;
            const desiredLeft = scrollX + centerXInChild;
            const desiredTop = scrollY + centerYInChild;
            const minLeft = scrollX + (elRect.width / 2) + margin;
            const maxLeft = scrollX + window.innerWidth - (elRect.width / 2) - margin;
            const minTop = scrollY + (elRect.height / 2) + margin;
            const maxTop = scrollY + window.innerHeight - (elRect.height / 2) - margin;
        ...
        function positionModalCenteredInViewport(el, onPositioned) {
            ...
            if (window.parent === window) {
                ...
                el.style.top = '50%';
                el.style.transform = 'translate(-50%, -50%)';
                if (onPositioned) requestAnimationFrame(onPositioned);
                return;
            }
            ...
            let timeoutId;
            const handleResponse = (messageEvent) => {
                const data = messageEvent && messageEvent.data;
                if (!data || data.type !== 'sac-parent-viewport' || data.requestId !== requestId) return;
                window.removeEventListener('message', handleResponse);
                if (timeoutId) clearTimeout(timeoutId);
                if (!el || !el.isConnected) return;
                applyParentViewportPositionTo(el, data);
                if (onPositioned) requestAnimationFrame(onPositioned);
            };
        ...
        function removePilotPreview() {
            ...
            if (!previewElement) hideDetailModalBackdrop();
        }
        ...
        function showPilotPreview(pilotId, pilotName, triggerElement) {
            ...
            // Center in viewport — handles both standalone and iframe embedding
            positionModalCenteredInViewport(pilotPreviewElement, function() {
                if (pilotPreviewElement) {
            ...
            if (detailedFlight) {
                ...
                detailedStatsHtml = \`
                    <div class="stats-section">
                        <div class="stats-header">Flight Performance \${isTaskMode ? 'DSC' : 'Free'}</div>
        ...
        if (!taskScoreHtml) {
            ...
            // Center in viewport — handles both standalone and iframe embedding
            positionModalCenteredInViewport(previewElement, function() {
                if (previewElement) {
        ...
        function hideFlightPreview(evt, immediate = false) {
            ...
            if (!pilotPreviewElement) hideDetailModalBackdrop();
        }
        ...
        function buildLeaderboard() {
            ...
            let visible = applyAgeCategoryFilters(applyUnder200Filter(applyClubFilter(leaderboard)));
            if (isSilverCGull) {
                const verification = pilot => pilotVerifications.dobVerifications?.[pilot.userId || pilot.pilotId];
                visible = visible.filter(pilot => verification(pilot)?.eligible !== false).sort((a, b) => {
                    const rank = pilot => isDobVerifiedEntry(verification(pilot)) && Number.isFinite(verification(pilot).awardRank)
                        ? verification(pilot).awardRank : Infinity;
                    return (rank(a) - rank(b)) || a.pilot.localeCompare(b.pilot);
                });
            }
            ...
            visible.forEach((pilot) => {
                ...
                let rankDisplay = 'N/A';
                const hasSilverRank = !isSilverCGull || Number.isFinite(pilotVerifications.dobVerifications?.[pilot.userId || pilot.pilotId]?.awardRank);
                if (isSilverCGull && !hasSilverRank) rankDisplay = '';
                if (!isBhcUnofficialRow && hasSilverRank) {
                    rankedPosition += 1;
                ...
                if (under200Enabled && !isSilverCGull) {
                    ...
                    if (isUserVerified) {
                        verificationBadge = '<div class="verification-badge verified">✓ Age verified' +
                            (Number.isFinite(verificationData.ageAtAchievement) ? ' - ' + verificationData.ageAtAchievement + ' at achievement' : '') + '</div>';
                        rowClass = 'verified-row';
                    } else {
                        verificationButton = ALLOW_PUBLIC_VERIFICATION_WRITE
        ...
        function calculateSilverCGullTrophy() {
            ...
            silverCGullLeaderboard.forEach(pilot => {
                ...
                if (verification?.eligible === false) return;
                if (isDobVerifiedEntry(verification) && Number.isFinite(verification.awardRank)) {
                    const ageAtAchievement = verification.ageAtAchievement;

                    if (verification.awardRank < youngestAge) {
                        youngestAge = verification.awardRank;
                        youngestVerified = {
                            ...pilot,
                            ageAtAchievement
                        };
        ...
        function showDOBVerificationForm(pilotId, pilotName) {
            ...
            overlay.innerHTML = \`
                <div class="verification-form">
                    ...
                    <label style="display:flex;align-items:flex-start;gap:8px;margin:16px 0;">
                        <input type="checkbox" id="silverDismiss" style="width:auto;" onchange="document.getElementById('dobInput').disabled=this.checked;document.getElementById('dobFields').hidden=this.checked;" />
                        <span>I received my Silver badge before this season. Remove me from the Silver C-Gull candidate list.</span>
                    </label>
                    <p>No date of birth is needed for removal. Confirm your request using the link emailed to you.</p>
                    <div id="dobFields">
                        <input type="date" id="dobInput" max="2010-12-31" />
                        <label for="dobInput">Date of Birth</label>
                        <p style="font-size:0.85em;">Your date of birth is used only to determine your age for the Silver C-Gull award. It is not displayed publicly or shared with anyone other than the <strong>SAC Awards coordinator</strong>. Your age at achievement may appear in the standings.</p>
                    </div>
                    <div>
        ...
        async function submitDOBVerification(pilotId, pilotName) {
            ...
            const dismiss = document.getElementById('silverDismiss').checked;
            const email = emailInput.value.trim().toLowerCase();

            if (!dismiss && !dateOfBirth) {
            ...
            try {
                await requestVerificationEmail({
                    type: dismiss ? 'silver-dismiss' : 'dob',
                    pilotId: pilotId,
                    pilotName: pilotName,
                    ...(dismiss ? {} : { dateOfBirth }),
                    email: email
                ...
                closeVerificationForm();
                alert(dismiss ? 'A confirmation link was emailed to you. Open it to remove yourself from the Silver C-Gull list.' :
                    'A confirmation link was emailed to you. Open it to confirm your date of birth.');
            } catch (error) {
        ...
        /* The pilot tooltip sits inside a .flight-preview wrapper — make that wrapper transparent */
        #pilot-preview.flight-preview {
        ...
        /* Dim backdrop shown behind flight + pilot detail popups */
